...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY INVOKER  VIEW `vwproductos` AS select  `cetecnet_mitienda`.`productos`.`id_Producto`  AS  `id_Producto` ,
 ...
 `cetecnet_mitienda`.`productos`.`foto`  AS  `foto`  from ( (  `cetecnet_mitienda`.`productos`  join  `cetecnet_mitienda`.`categorias`  on ( (  `cetecnet_mitienda`.`productos`.`id_Categoria`  =  `cetecnet_mitienda`.`categorias`.`id_Categoria`  ) )  ) join  `cetecnet_mitienda`.`marcas`  on ( (  `cetecnet_mitienda`.`productos`.`id_Marca`  =  `cetecnet_mitienda`.`marcas`.`id_Marca`  ) )  ) order  by  `cetecnet_mitienda`.`productos`.`id_Producto` ;
